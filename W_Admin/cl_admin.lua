...
function agivemoney()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('W_admin:givecash', amount) 
		end
	end
end
function agivemoneybank()
	...
	if amount ~= nil then
		amount = tonumber(amount)
		if type(amount) == 'number' then
			TriggerServerEvent('W_admin:givecashbank', amount)
		end
	end
end
function agivemoneysale()
	...
	if amount ~= nil then
		amount = tonumber(amount)
		if type(amount) == 'number' then
			TriggerServerEvent('W_admin:givecashsale', amount)
		end
	end
end
...
RegisterNetEvent('W_admin:bringplayer')
AddEventHandler('W_admin:bringplayer', function(plyPedCoords)
...
function tpplayertome()
	...
	if plyId ~= nil then
		...
		if type(plyId) == 'number' then
			...
			TriggerServerEvent('W_admin:bringplayer', plyId, plyPedCoords)
		end
	end
end
...
local JoueurMenu = {	
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {255, 0, 0}, Title = "joueur" },
	...
	Events = {
		onSelected = function(self, _, btn, PMenu, menuData, currentButton, currentBtn, currentSlt, result, slide)
			...
			elseif btn == "Téléporter le joueur à nous" then
				tpplayertome(v)  
			elseif btn == "Exclure la personne (Raison)" then 
	...
	Menu = {
		["Joueur"] = {
			...
			b = {
				...
				{name = "Téléporter le joueur à nous"},
				{name = "Exclure la personne (Raison)"}
			}
		}
	}
}
...
local menuadmin = {
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {255, 0, 0}, Title = "Superadmin"},
...
local menuadmin2= { 
	Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {255, 0, 0}, Title = "Admin"},
	Data = { currentMenu = "Options"},
	Events = { onSelected = OnSelected },
	Menu = {
		["Options"] = {
